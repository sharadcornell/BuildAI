...
// Phase 3A — AI access control-plane loaders.
...
// NOTE: this table holds METADATA + a MASKED key hint only — never a raw key.

export type AiAccessStatus = "pending" | "active" | "suspended" | "revoked";
...
export async function getStudentAiAccess(): Promise<StudentAiAccess | null> {
  ...
  try {
    const { data } = await supabase
      ...
      .select(
        "status, litellm_virtual_key_hint, monthly_budget_usd, allowed_models, issued_at, revoked_at",
      )
      .eq("user_id", user.id)
      .maybeSingle();
...
export type AdminAiAccessOverview = {
  ...
  counts: { pending: number; active: number; suspended: number; revoked: number; total: number };
...
/**
 * Admin overview of AI access metadata: per-status counts, recent records, the
 * LiteLLM/Langfuse config status, and the list of students who don't have a
 * record yet (so an admin can create one). RLS-scoped to admins; error-safe.
 */
export async function getAdminAiAccessOverview(): Promise<AdminAiAccessOverview> {
  const base: AdminAiAccessOverview = {
    ...
    counts: { pending: 0, active: 0, suspended: 0, revoked: 0, total: 0 },
    recent: [],
    unassigned: [],
  };
  ...
  try {
    const [total, pending, active, suspended, revoked, recent, students] =
      await Promise.all([
        headCount(),
        headCount("pending"),
        headCount("active"),
        ...
        supabase
          .from("student_ai_access")
          .select(
            "id, status, litellm_virtual_key_hint, monthly_budget_usd, created_at, profile:profiles ( full_name )",
          )
    ...
    base.counts = { pending, active, suspended, revoked, total };
    ...
    base.recent = (recent.data ?? []).map((r) => {
      ...
      return {
        ...
        monthlyBudgetUsd: (r.monthly_budget_usd as number | null) ?? null,
